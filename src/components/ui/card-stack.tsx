  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: isVertical ? Math.max(580, cardHeight + 120) : Math.max(380, cardHeight + 80) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
        ...
        >
          <AnimatePresence initial={false}>
            {items.map((item, i) => {
              ...
              const x = isVertical ? (abs * 12) : (off * cardSpacing);
              const y = isVertical ? (off * cardSpacing) : (abs * 10);
              const z = -abs * depthPx;
              ...
              // Lift direction based on orientation
              const translateX = isVertical && isActive ? activeLiftPx : x;
              const translateY = isVertical ? y : (y + lift);
              ...
              const dragProps = isActive
                ? {
                    ...
                    onDragEnd: (
                      ...
                      if (reduceMotion) return;
                      const travel = isVertical ? info.offset.y : info.offset.x;
                      const v = isVertical ? info.velocity.y : info.velocity.x;
                      const threshold = Math.min(160, (isVertical ? cardHeight : cardWidth) * 0.22);
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-[2.5rem] border-4 border-black/10 dark:border-white/10 overflow-hidden shadow-xl",
...
function DefaultFanCard({ item }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full bg-[#121624]">
      {/* image */}
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-8">
        ...
        {item.description ? (
          <div className="mt-2 line-clamp-2 text-sm font-medium text-slate-300">
            {item.description}
          </div>
        ) : null}
      </div>
